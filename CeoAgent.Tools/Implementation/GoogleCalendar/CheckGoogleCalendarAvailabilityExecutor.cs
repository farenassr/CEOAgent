    ...
    ToolExecutionGatewayHelper helper) : IToolExecutor
{
    ...
    {
        if (!ToolExecutionGatewayHelper.TryDeserialize<CheckAvailabilityRequest>(request.ToolCall.Arguments, out var arguments)
        ...
        var execution = await executor.CheckAvailabilityAsync(
            request.CompanyId,
