    ...
    ILogger<WhatsAppWebhookIngestionService> logger)
{
        ...
        ParsedWhatsAppMessage message)
    {
        ...
        if (message.Type == "audio" && !string.IsNullOrWhiteSpace(message.MediaId))
        {
            inbound.Payload.Audio = new AudioPayload
            {
                BlobUri = $"whatsapp-media://{message.MediaId}",
                ContentType = message.MediaMimeType ?? "application/octet-stream",
                SizeBytes = 0,
        ...
        foreach (var entry in entries.EnumerateArray())
        {
            ...
            foreach (var change in changes.EnumerateArray())
            {
                ...
                var text = type == "text" && message.TryGetProperty("text", out var textElement)
                    ...
                    ? textBody.GetString()
                    : null;
                var mediaId = type == "audio" && message.TryGetProperty("audio", out var audioElement)
                    && audioElement.TryGetProperty("id", out var audioId)
                    ? audioId.GetString()
                    : null;
                var mediaMimeType = type == "audio" && message.TryGetProperty("audio", out var audioMetadata)
                    && audioMetadata.TryGetProperty("mime_type", out var mimeType)
                        ? mimeType.GetString()
                        : null;
